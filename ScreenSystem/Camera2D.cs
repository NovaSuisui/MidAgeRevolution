...
namespace MidAgeRevolution.ScreenSystem
{
    public class Camera2D
    {
        ...
        {
            return Matrix.CreateOrthographic(-64, 36, 0f, 1f);
        }
        public static Matrix GetView()
        {
            var cameraPosition = new Vector3(new Vector2(64 / 2, 36 / 2), 0f);
            var cameraUp = Vector3.TransformNormal(Vector3.Down, Matrix.CreateRotationZ(0f));
            return Matrix.CreateLookAt(cameraPosition, cameraPosition + Vector3.Forward, cameraUp);
        }
    }
}